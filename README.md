...
> 자동차 극장의 위치 및 간단한 정보를 한눈에 볼 수 있으며 영화를 예매하고 결제할 수 있는 사이트를 구현한 JSP MVC Model2 프로젝트
...
- 개발 인원 : 6명(팀프로젝트)
- 맡은 역할 : 메인페이지(50%), 예매(100%), 결제(100%), 예매내역(100%)
...
💡 예매페이지
- [ 지역 > 극장 > 영화 > 시간 ] ajax 비동기 처리로 영화정보를 JDBC를 활용하여 해당하는 내용을 구현
...
- 예매내역 데이터 조회후 -> 해당 데이터 페이징처리 + 리스트형식으로 출력.
